...
boost::shared_ptr<vublock> vuFactory(iter start){
	...
	switch (start[1]){
		...
		case Activities::TREP: 	return p(new Activities(start));
		case Faults::TREP: 	return p(new Faults(start));
		case Speeds::TREP: 	return p(new Speeds(start));
		case Technical::TREP: 	return p(new Technical(start));
		default:
