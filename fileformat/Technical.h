...
class Technical : public vublock {
	...
	void CompleteReport(reporter& report) const {
		...
		for(reporter::subblock b = report.newsub("VuCalibrationRecord",
				IntByte()); b(); ++b) {
			...
			report("wVehicleCharacteristicConstant", stringify(Int16()) + " "
					+ tr("Impulses") + "/km");
			report("kConstantOfRecordingEquipment", stringify(Int16()) + " "
					+ tr("Impulses") + "/km");
			int lTyreCircumference = Int16();
			report("lTyreCircumference", stringify(lTyreCircumference / 8)
					+ " " + (lTyreCircumference % 8 ? stringify(
					lTyreCircumference % 8) + "/8 " : "") + "mm");
			report("tyreSize", fixedString(15));
			report("authorisedSpeed", stringify(IntByte()) + " km/h");
			report("oldOdometerValue", stringify(Odometer()) + " km");
			report("newOdometerValue", stringify(Odometer()) + " km");
