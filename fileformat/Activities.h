...
class Activities : public vublock {
	...
	Activities(iter nstart) :
		vublock(nstart), driven(0), day(BEInt32(start + 2)) {
		Init();
	...
	void CompleteReport(reporter& report) const {
		...
		for(reporter::subblock b = report.newsub("VuCardIWRecord", Int16()); b(); ++b) {
			...
			report("previousVehicleRegistrationNation",
					nationNumeric(IntByte()));
		...
		for(reporter::subblock b =
				report.newsub("activityChangeInfos", Int16()); b(); ++b) {
		...
		for(reporter::subblock b = report.newsub("SpecificConditionRecord",
				Int16()); b(); ++b) {
			report("entryTime", readDate().str());
	...
	void BriefReport(reporter& report) const {
		for(unsigned int j = 0; j < CardInOuts.size(); ++j) {
			...
			report("Driven distance", stringify(t.OdometerOut - t.OdometerIn)
					+ " km");
		}
		report << DayAct;
