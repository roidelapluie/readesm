...
class CAid {
	...
	bool operator==(const CAid& other) const {
		return numNation == other.numNation && alphaNation == other.alphaNation
				&& serialNumber == other.serialNumber && additionalCoding
				== other.additionalCoding && identifier == other.identifier;
	}
	bool operator!=(const CAid& other) const {
		return !operator==(other);
	}
	CAid(iter start) :
		numNation(start[0]), alphaNation(fixedString(start + 1, 3)),
				serialNumber(start[4]), additionalCoding(BEInt16(start + 5)),
				identifier(start[7]) {
	}
...
bool CheckSignature(const iter& data, int length, const iter& signature,
		int siglength, const rsa& key) {
	std::vector<unsigned char> decryptedsig = key.perform(&signature[0],
			siglength);
