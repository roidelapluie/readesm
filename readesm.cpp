...
/**
 ...
 This program takes an ESM file, downloaded from a digital tachograph or a
 driver card, and converts it into human-readable form, either html or simple
 plaintext. Since the data format is taken from an european community
 regulation (OJ:L:2002:207, p. 1-252,
 http://eur-lex.europa.eu/LexUriServ/LexUriServ.do?uri=OJ:L:2002:207:0001:0252:EN:PDF),
 I doubt it is of any use for drivers or companies outside of the european union.

 \section intro_comp Compiling
 Since so far there is no binary distribution for the program, you will have to
 compile it yourself. To do that, you need a c++ compiler, parts of the boost
 (http://www.boost.org/) library (specifically program_options and shared_ptr)
 and the Gnu MP library (http://gmplib.org/), wich is used to check the various
 RSA signatures. Once all those are installed, typing make/make install should
 work fine.

 \section intro_run Running the program
 In most circumstances, you will run the program from the commandline:
 readesm --infile foo.esm --outfile bar.html
...
int main(int argc, char* argv[]) {
	...
	po::options_description desc(InfoAppend(false, false) + " " + tr(
			"ESM Reader, allowed options"));
	desc.add_options()("help,h", trc("produce help message"))("infile,i",
			po::value<string>(), trc("input file"))("outfile,o", po::value<
			string>()->default_value("-"), trc("output file"))("verbose,v",
			po::value<bool>()->default_value(false), trc(
					"verbose output format, print simply everything"))(
			"format,f", po::value<string>()->default_value("text"), trc(
