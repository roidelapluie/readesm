...
class txtreporter : public reporter {
	public:
	txtreporter(const string& title_ = "ESM Data") :
		reporter(title_) {
	}
	virtual string str() {
...
class htmlreporter : public reporter {
	...
	htmlreporter(const string& title_ = "ESM Data") :
		reporter(title_), targetcount(0) {
	}
	virtual string str() {
		ostringstream o;
		o
				<< "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n<html xmlns=\"http://www.w3.org/1999/xhtml\" dir=\"ltr\" lang=\"en-US\">\n<head><link rel='stylesheet' type='text/css' media='screen' href='style.css'/><title>"
				<< title
				<< "</title><meta http-equiv='Content-Type' content='text/html; charset=utf8'/></head><body><h1>"
				<< title << "</h1>" << links.str() << "<hr/>"
				<< reporter::str() << "<hr/>" << InfoAppend(true)
				<< "</body></html>\n";
		return o.str();
	}
	virtual void bigblockstart(const string& name) {
		links << "<a href='#" << stringify(++targetcount) << "'>" << tr(name)
				<< "</a><br/>";
		(*this) << "<h2><a name='" << stringify(targetcount) << "'>"
				<< tr(name) << "</a></h2><table>";
	}
	virtual void bigblockend() {
		(*this) << "</table>";
	...
	virtual void single(const string& description, bool ishead = false) {
		string mark = "td";
		if(ishead) mark = "th";
		(*this) << "<tr><" << mark << " colspan='2' class='center'>" << tr(
				description) << "</" << mark << "></tr>" << std::endl;
	}
	virtual void operator()(const string& description, const std::string& value) {
		(*this) << "<tr><th>" << tr(description) << "</th><td>" << value
				<< "</td></tr>\n";
	}
	virtual void operator()(const string& description, int value) {
		(*this) << "<tr><th>" << tr(description) << "</th><td>" << value
